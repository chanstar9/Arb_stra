...
for mkt_time, ELW in A3021.iterrows():
    if ELW["volume"] >= 2500 * spread_num:
        ...
        if (_df["매수1단계우선호가가격"] * 100 < ELW["Price"]) & (_df["매도1단계우선호가가격"] * 100 > ELW["Price"]):
            if abs(_df["매수1단계우선호가가격"] * 100 - ELW["Price"]) > abs(_df["매도1단계우선호가가격"] * 100 - ELW["Price"]):
                # ELW 매수 포지션, option 매도 포지션
            ...
            if abs(_df["매수1단계우선호가가격"] * 100 - ELW["Price"]) < abs(_df["매도1단계우선호가가격"] * 100 - ELW["Price"]):
                ...
                account2.append([mkt_time, ELW["Price"], -2500 * spread_num, amt / spread_num, spread_num])
            if abs(_df["매수1단계우선호가가격"] * 100 - ELW["Price"]) == abs(_df["매도1단계우선호가가격"] * 100 - ELW["Price"]):
                if _df["매수1단계우선호가잔량"] * 100 > _df["매도1단계우선호가잔량"] * 100:
                    # ELW 매도 포지션, option 매수 포지션
                    account2.append([mkt_time, ELW["Price"], -2500 * spread_num, amt / spread_num, spread_num])
                else:
                    # ELW 매수 포지션, option 매도 포지션
                    account2.append([mkt_time, ELW["Price"], 2500 * spread_num, amt / spread_num, -spread_num])
        if _df["매수1단계우선호가가격"] * 100 > ELW["Price"]:
            # ELW 매수 포지션, option 매도 포지션
            account2.append([mkt_time, ELW["Price"], 2500 * spread_num, amt / spread_num, -spread_num])
        if _df["매도1단계우선호가가격"] * 100 < ELW["Price"]:
            # ELW 매도 포지션, option 매수 포지션
            account2.append([mkt_time, ELW["Price"], -2500 * spread_num, amt / spread_num, spread_num])
